...
class Seq2SeqFusion(nn.Module):
    ...
    def forward(self, x):
        batch_size, seq_len, input_size = x.shape
        h, c = self.Encoder(x)
        outputs = torch.zeros(batch_size, seq_len, input_size).to(self.device)
        _input = torch.zeros(batch_size, input_size).to(self.device)
        for i in range(self.output_size):
            output, h, c = self.Decoder(_input, h, c)
            outputs[:, i, :] = output
            _input = output
        accumulator = torch.zeros(batch_size, input_size).to(self.device)
        for i in range(self.output_size):
            accumulator += outputs[:, i, :]
        out = self.linear(accumulator)
